...
export async function POST(request: NextRequest) {
    const { topic, parentTopic, parentSummary, keepTopic } = await request.json();
    if (!keepTopic) {
        const node = await getNodeWithChildren(topic, parentTopic, parentSummary, false);
        return NextResponse.json(node);
    }
    const node = await getNodeWithChildren(topic, parentTopic, parentSummary, keepTopic);
    // const node = {
